...
const execute = async ({ method, path = '', body = null, query = {}, headers = {}, forceMigrationLastUpdate = null, skipDecrypt = false } = {}) => {
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        if (res?.error?.message) {
          toast?.error(res?.error?.message, { autoClose: import.meta.env.TEST_PLAYWRIGHT !== 'true' });
        } else if (res?.error) {
          toast?.error(res?.error, { autoClose: import.meta.env.TEST_PLAYWRIGHT !== 'true' });
        } else if (res?.code) {
          toast?.error(res?.code, { autoClose: import.meta.env.TEST_PLAYWRIGHT !== 'true' });
        } else {
    ...
    if (typeof errorExecuteApi === 'string') {
      toast.error(errorExecuteApi, { autoClose: import.meta.env.TEST_PLAYWRIGHT !== 'true' });
    } else if (errorExecuteApi?.message) {
      toast.error(errorExecuteApi.message, { autoClose: import.meta.env.TEST_PLAYWRIGHT !== 'true' });
    } else {
      toast.error('Désolé, une erreur est survenue', { autoClose: import.meta.env.TEST_PLAYWRIGHT !== 'true' });
    }
