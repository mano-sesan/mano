        ...
        {days.map((day) => {
          return (
            <details open={days.length === 1} className="tw-my-2 tw-p-2" key={day}>
              <summary>
                <h4 className="tw-inline-block tw-text-base tw-capitalize tw-mb-2">{dayjsInstance(day).format("dddd D MMM")}</h4>
              </summary>
...
function Transmission({ report, team, day, teamId, userId, reactSelectInputId, onOpenTransmissionModal }) {
  ...
  return (
    <>
      <div className="tw-p-4 tw-mb-4 tw-flex tw-flex-col tw-gap-2 tw-rounded-2xl tw-bg-gray-100">
        <p className="tw-font-medium tw-flex tw-items-center tw-gap-1">
          {team?.nightSession ? <MoonIcon className="tw-h-5 tw-w-5 tw-text-blue-900" /> : <SunIcon className="tw-h-5 tw-w-5 tw-text-yellow-500" />}{" "}
          {team?.name || ""}
        </p>
