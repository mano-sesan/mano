...
test("test", async ({ page }) => {
  ...
  await page.locator("#encryptionKey").fill("plafplafplaf");
  await page.getByLabel("Confirmez la clé de chiffrement").click();
  await page.getByLabel("Confirmez la clé de chiffrement").fill("plafplafplaf");
  ...
  await loginWith(page, "admin1@example.org", "secret", "plafplafplaf");
  await page.getByRole("link", { name: "Agenda" }).click();
  ...
  await page.getByRole("button", { name: "Changer la clé de chiffrement" }).click();
  await page.locator("#encryptionKey").click();
  await page.locator("#encryptionKey").fill("ploufploufplouf");
  await page.getByLabel("Confirmez la clé de chiffrement").click();
  await page.getByLabel("Confirmez la clé de chiffrement").fill("ploufploufplouf");
  await page.locator('[data-test-id="encryption-modal"]').getByRole("button", { name: "Changer la clé de chiffrement" }).click();
  await page.getByText("Données chiffrées ! Veuillez noter la clé puis vous reconnecter").click();
  await page.locator('[data-test-id="encryption-modal"]').getByRole("button", { name: "Close" }).click();
  await logOut(page, "User Admin Test - 1");
  await loginWith(page, "admin1@example.org", "secret", "ploufploufplouf");
  await page.getByRole("link", { name: "Personnes suivies" }).click();
