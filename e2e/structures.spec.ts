...
test("Contacts", async ({ page }) => {
  ...
  await page.getByLabel("Nom").fill("Contact 1");
  ...
  await page.getByLabel("Nom").fill("Contact 1");
  await page.getByRole("button", { name: "Enregistrer" }).click();
  await page.getByText("Un contact avec le même nom existe déjà").click();
  await page.getByLabel("Nom").fill("Contact 2");
  await page.getByRole("button", { name: "Enregistrer" }).click();
  await page.getByText("Contact créé !").click();
  ...
  await page.locator("#categories").fill("infirmerie");
  await page.locator("#categories").press("Enter");
  await page.getByText("Contact mis à jour !").click();
});
