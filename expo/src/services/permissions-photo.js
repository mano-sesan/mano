...
export const alertPhotosSetting = async (error, requestedAccess = "images") => {
  try {
    ...
    if (showAlert) {
      Alert.alert(
        `Mano n'a pas accès à ${requestedAccess === "camera" ? "votre appareil photo" : "vos images"}`,
        `Cliquez sur Réglages pour permettre à Mano d'accéder à ${requestedAccess === "camera" ? "votre appareil photo" : "vos images"}`,
        [
          {
            text: "Réglages",
            onPress: () => {
