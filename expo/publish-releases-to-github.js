...
const nantesApkPath = path.join(__dirname, "mano-nantes.apk");
const publishApksToGithub = async () => {
  ...
  if (!fs.existsSync(nantesApkPath)) {
    console.log(chalk.red("Error: Nantes APK not found at"), nantesApkPath);
    console.log(chalk.yellow("Please run `yarn build-local:android-apks` first"));
    process.exit(1);
  }
  ...
  try {
    ...
    if (nantesResult.stderr?.length) {
      console.log(chalk.red("Error uploading Nantes APK:"), chalk.bgRed(nantesResult.stderr));
    }
    if (nantesResult.stdout?.length) {
      console.log(chalk.green("Success uploading Nantes APK:"), chalk.green(nantesResult.stdout));
    }
    ...
    console.log(chalk.yellow("Nantes release: https://mano.sesan.fr/download-nantes"));
  } catch (error) {
